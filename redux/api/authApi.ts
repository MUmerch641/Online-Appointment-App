    ...
    registerUser: builder.mutation({
      query: (userData: RegisterUserRequest) => {
        ...
        if (!userData.projectId || userData.projectId.length !== 24) {
          console.error(`Invalid project ID format: "${userData.projectId}", length: ${userData.projectId?.length}`);
          throw {
            status: 400,
            data: { message: 'Invalid project ID format. Expected 24-character hex string.' }
          };
        }
